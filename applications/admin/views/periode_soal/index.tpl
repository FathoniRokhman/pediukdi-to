...
{block name='content'}
	...
	<p>
		...
		<a href="{site_url('periode-soal/add')}/{$periode->id_periode}" class="btn btn-xs btn-primary">Tambah</a>
	</p>
		...
		<tbody>
			{foreach $data_set as $data}
				<tr>
					...
					<td>
						<a href="{site_url('periode-soal/edit')}/{$data->id_periode_soal}" class="btn btn-xs btn-info">
							<span class="glyphicon glyphicon-pencil"></span> Edit
						</a>
						<a href="#" class="btn btn-xs btn-danger" data-toggle="modal" data-target="#deleteModal" data-id="{$data->id_periode_soal}">
	...
	<script type="text/javascript">
		$(document).ready(function() {
			$('#deleteModal').on('show.bs.modal', function(event) {
				...
				$(this).find('#formDelete').attr('action', "{site_url('periode-soal/delete/')}" + button.data('id'));
			});
		});
	</script>
{/block}