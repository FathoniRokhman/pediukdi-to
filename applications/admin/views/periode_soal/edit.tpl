	...
	<form class="form-horizontal" action="{current_url()}" method="post">
		<fieldset>
			...
			<div class="form-group">
				...
				<div class="col-lg-6">
					{html_select_date prefix="tgl_mulai_" time=$data->tgl_mulai field_order="DMY" end_year="+5" all_extra='class="form-control" style="width: auto; display: inline-block"'}
				</div>
			</div>
			...
			<div class="form-group">
				...
				<div class="col-lg-6">
					<select name="id_form_soal" class="form-control" style="width: auto">
						...
						<option value="{$form_soal->id_form_soal}" {if $form_soal->id_form_soal == $data->id_form_soal}selected{/if}>{$form_soal->nama_form}</option>
						{/foreach}
					</select>
				</div>
			</div>
